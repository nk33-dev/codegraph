...
当前没有在本工作区内伪造这些数字。`v1.6.0-personal.1` 只交付 GitHub `.tgz`，不等待三平台工作流，也不代表真实仓库 Agent A/B、能耗测量或 npm registry 发布已经完成。
...
这些结果只代表本机。新增 GitHub 工作流尚未推送执行，因此不能写成 Windows/Linux/macOS 矩阵已通过。
