...
发行提交 `f4a22e6` 的运行 `35190011391` 在 Windows 高负载 runner 上再次暴露文件级并发过量：单测耗时从亚秒级膨胀到 15～34 秒，超时任务继续持有 SQLite 并写入进程指标，造成多条次生失败。CI 因此改为 Windows 单 worker，Linux/macOS 保持 4 worker；这属于门禁稳定性修复，不改变运行时功能，也不覆盖已发布标签。
