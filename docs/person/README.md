...
待发布版本为 [v1.6.0-personal.8](releases/v1.6.0-personal.8.md)：修复 blast radius 调用方计数与所列文件不一致、要求“相关测试”时整篇渲染测试源码，并让 MCP 固定表面预算跟上 explore 新增的 `includeTestSource` 选项。发布前的本机定向验证与 CI 结论见[开发验证记录](test-repairs.md)，版本状态以 GitHub Release 页面为准。
...
个人扩展通过 `personal` 分支维护；首个 GitHub prerelease 为 `v1.6.0-personal.1`，当前发行版本为 `v1.6.0-personal.7`，待发布版本为 `v1.6.0-personal.8`。向 npm registry 安装上游包不会获得个人改动；个人版按[安装说明](personal-usage.md)从 GitHub Release `.tgz` 或固定标签安装。
