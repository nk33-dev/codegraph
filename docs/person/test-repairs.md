...
- 文档：新增 [MCP 时延、常驻加载与 Read 回退](mcp-latency-and-load.md)；`docs/retrieval.md` 的 explore 预算表更正为当前 13K/18K/24K 档位（旧值 28K/35K/38K 标注为历史）。该阶段测量值为 2,214/4,977/7,191，最终合入 P0 后为 2,214/4,810/7,024。
...
两次握手测量都用当时 `tsc` 生成的 `dist/`，返回的静态表面都是 2 个工具、4,977 字符 / 4,983 字节（差额来自 3 个非 ASCII 字符）。后续 P0 继续压缩 schema，因此该数字只对应测量时构建；启动耗时读数仍作为 P2 问题 11 的历史依据。

`scripts/test-changed.mjs` 的第一次运行有 1 项失败：`explore-cross-call-dedup.test.ts > leaves the first call of a session untouched`。前一个用例改写并还原文件后没有等待索引恢复，watcher 防抖窗口会污染后续按字节比较。最终合并态在还原后显式执行 `cg.sync()`，修复测试隔离，不放宽业务断言。
...
- 精确意图查询显式合并共用 Graph 入边，动态 namespace 调用方不再被通用遍历预算漏掉；输出按定义、直接调用方和测试分类。
...
- **personal.7 最终发行**：提交 `006a3ec` 的三平台 CI `35302308070` 全绿；Personal Release `35302838636` 完成隔离安装、归档、`SHA256SUMS`、标签和 prerelease，标签与资产均指向该提交。
...
以下数字来自原 `change-context.md` 的历史 Windows 验证，不是本次修复重新运行的结果：完整构建通过；阶段四专项 6 项通过；Graph/MCP 定向回归 47 项通过；全量测试 272 个文件通过、21 个按环境跳过，4649 项通过、234 项跳过；其后新增的分支切换专项单独通过，未重复全量测试。

## 2026-09-23 Claude Code 体验问题修复

- 实现范围与逐项结论见 [MCP 体验问题处理记录](mcp-experience-review.md)，当前契约已同步结构化查询、改动上下文、流程证据、索引状态和统一路由文档。版本未修改，尚未发布。
- Windows 本地 `npm run typecheck` 通过。定向回归覆盖 MCP 文本传输、错名建议、排序、HEAD、换行符、Hook、全文索引升级及补扫、LSP 非空但未就绪、真实路由和性能操作次数。源码 CLI 通过已安装的 vite-node 运行，验证聊天 Hook 静默及 `explore --json`，未构建 dist。
- 主要通过批次：查询/LSP/路由 59 项（8 个产物相关用例排除）；FTS/符号/Hook 等 83 项（5 个产物用例条件跳过）；最终体验/全文/脱敏/表面预算/Windows 子进程 66 项；会话/过期源码/blast radius/全文 51 项；路由错误上下文及错名补验 5 项；补扫预算与合并分页 1 项。批次有重叠，不合并为独立测试总数。编辑文本引擎 17 项、流程意图两组 14 项及改动上下文 6 项也已通过。
- 首轮 `npm run check:quick` 类型检查通过，但自动选择 193 个测试文件后出现新增元数据写入遗漏 `updated_at` 的约束错误，因此中止该批次。修复后使用上述定向测试验证，未把中止的批量检查记为通过，也未重新运行完整构建或全量测试。
- 复杂度回归确认 1,000 边的影响分析访问次数低于 10,000，避免原先约 100 万次的边扫描；100 个改动文件汇合到同一测试时只分类读取一次。该结果是确定性操作计数，不代表实测真实仓库耗时收益。
- 新增补扫缓存表由文件索引初始化幂等建立；旧词 FTS 在事务内替换为 trigram，升级测试使用临时数据库副本。无 FTS5 时保持字面检索；大文件覆盖清单在完整 sync 后建立，旧清单未就绪时明确警告。
- 尚未完成真实 Claude Code 宿主展示、真实语言服务覆盖、三平台 CI、真实仓库检索 A/B 和发行安装验证。报告第 10 项按其建议保留观察，不宣称不相关文件召回已全面解决。

## 2026-09-23 个人版 .11 发布门禁与清理修复

- 发布准备提交 `7363f62` 已推送一次；CI `35860358994` 的 Ubuntu/macOS 构建与全量测试通过。Windows 第一次为握手测试退出后删除目录 `EBUSY`（4790 项通过、1 项失败）；同提交重跑为 roots 测试目录和 WAL 测试数据库清理 `EBUSY`（4789 项通过、2 项失败）。未触发 Personal Release。
- roots 测试固定直连服务并关闭 WASM 重启 shim；握手与 roots 清理在等待进程退出后使用 Node 原生异步删除及有界重试，不吞掉最终失败。
