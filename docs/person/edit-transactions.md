...
- 每个文件提交前重新核对原内容和目标是否出现；回滚只撤销能归属于本事务的内容。暂存后出现的外部修改会保留并标为 `recovery_required`，不会用旧备份覆盖编辑器的新内容；尚未写出的 create/rename 目标不会被回滚删除。
...
恢复目标是进程中断与可识别的文件冲突，不承诺操作系统断电持久性，也不能阻止不遵守事务锁的外部程序在最后校验与文件替换之间写入。事务锁遇到刚创建的空文件不会立即抢占，事务本身抛出的 `EEXIST` 不会触发整笔写入重试。
