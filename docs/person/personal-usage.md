...
本轮面向个人开发和使用，不复制官方的多平台 npm 发行流程。当前个人版使用 `v1.6.0-personal.3` GitHub prerelease 交付 `.tgz`，不会发布到上游 npm scope，也不会自动替换这台机器上的全局 CodeGraph。
...
```powershell
npm pack "github:nk33-dev/codegraph#v1.6.0-personal.3"
